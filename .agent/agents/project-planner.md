...
7. **Create `docs/plans/YYYYMMDD_{task-slug}.md` (MANDATORY for PLANNING mode)**
8. **Verify plan file exists in `docs/plans/` before exiting (PLANNING mode CHECKPOINT)**

---

## 🔴 PLAN FILE NAMING (DYNAMIC)

> **Plan files are named based on the date and task, stored in `docs/plans/`.**

### Naming Convention

| User Request | Plan File Name (Example for 2026-02-11) |
|--------------|-----------------------------------------|
| "e-commerce site with cart" | `20260211_ecommerce-cart.md` |
| "add dark mode feature" | `20260211_dark-mode.md` |
| "fix login bug" | `20260211_login-fix.md` |
| "mobile fitness app" | `20260211_fitness-app.md` |

### Naming Rules

1. **Prefix:** `YYYYMMDD_` (Current UTC date)
2. **Slug:** Extract 2-3 key words from the request
3. **Format:** Lowercase, hyphen-separated (kebab-case)
4. **Max 30 characters** for the slug part
5. **No special characters** except hyphen
6. **Location:** `docs/plans/` (MANDATORY)
...
Slug:         dashboard-analytics
                    ↓
File:         docs/plans/20260211_dashboard-analytics.md
```
...
**Plan Storage (For PLANNING Mode):** `docs/plans/YYYYMMDD_{task-slug}.md`

```bash
# File name based on date and task:
# "e-commerce site" (2026-02-11) → docs/plans/20260211_ecommerce-site.md
```

> 🔴 **Location:** `docs/plans/` folder - NOT project root.
...
**EXIT GATE:**
```
272: [IF PLANNING MODE]
273: [OK] Plan file written to docs/plans/YYYYMMDD_{slug}.md
274: [OK] Read docs/plans/YYYYMMDD_{slug}.md returns content
275: [OK] All required sections present
276: → ONLY THEN can you exit planning.
...
| 8 | **DYNAMIC NAMING** | `docs/plans/YYYYMMDD_{slug}.md` | Easy to find, multiple plans OK |
